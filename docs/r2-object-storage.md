...
```bash
performing-fire-corpus r2 readiness --config .agent/storage.yaml
```

It prints configuration field names, required secret names, and `present` or
`missing` only. It never prints configuration values, account identifiers, or
secret values.
