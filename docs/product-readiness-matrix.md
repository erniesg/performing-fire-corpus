...
| ANTIEGG catalogue expansion beyond the one article | `inventory-public --source antiegg-posts` | `src/performing_fire_corpus/antiegg_metadata_adapters.py`, `antiegg_posts_inventory.py` | `tests/test_antiegg_metadata_adapters.py`, `tests/test_antiegg_posts_inventory.py`; `tests/fixtures/antiegg/` | `network-acquisition` | Governance and REST v2 shape reviewed 2026-07-26 (robots grants blanket crawl; `/wp-json/wp/v2/posts` returns 200 unauthenticated with `x-wp-total: 1463`). The portable fixture audit reaches two stable, prose-free records, terminates on `x-wp-totalpages`, and reports no unvisited remainder. The CLI lane paginates at `per_page=100` and reports `complete` only when unique ids retrieved equal the declared `x-wp-total`. No live run has been recorded. Prior blocked proof: `docs/antiegg-inventory-proof.md`. | `implemented-offline` (shape-bound fixture); no live proof | run a separately authorized bounded live inventory |
| ANTIEGG sitemap inventory | none | `src/performing_fire_corpus/antiegg_metadata_adapters.py` | `tests/test_antiegg_metadata_adapters.py` | `network-acquisition` | `ANTIEGGSitemapAdapter` raises `SourceShapeUnreviewed` before emitting a request, so no CLI path drives it. Blocker: the current sitemap shape has had no bounded review. | `held` | review the sitemap shape independently |
...
| NJP Video Library inventory | none | `src/performing_fire_corpus/njp_video_library_adapter.py` | `tests/test_njp_video_library_adapter.py` | `network-acquisition` | Governance recorded 2026-07-26, but the bounded run stopped before any catalogue request on `robots_ambiguous`: `/robots.txt` returns the SPA `index.html` (`text/html`), not a robots policy, so the adapter refused to interpret it. All catalogue pages remain unvisited. See `docs/njp-video-library-inventory.md`. | `held` | resolve the ambiguous robots response, then bind the adapter |
...
| Corpus evaluation (completeness, quality, duplicates, coverage) | none | `src/performing_fire_corpus/evaluation.py` | `tests/test_corpus_evaluation.py` | `portable` | Offline over sanitized snapshots. `docs/corpus-evaluation.md` records the current aggregate result: no source is a whole-source total, three sources are blocked, and no corpus index or selection manifest exists, so those sections are empty rather than clean. | `implemented-offline` | resolve the three durable source blockers |
...
The repository contains one historical live source proof, two current bounded
endpoint proofs, and dated blocked preflight proofs. A blocked preflight is live
...
| Issue 95 NJP Center Video Archive inventory | One independent exact-clean-head trusted-VM request pair at the pinned stage-two bounds. It retained eight unique PDF-link metadata records after an exact live/reviewed structure-digest match, with no blocker, laptop fallback, or linked-object request. | `docs/njp-center-video-archive-inventory-report.json` |
...
observed mediaObjects endpoint and bounds. Issue 95 independently establishes
completeness only for the observed Video Archive endpoint and bounds. Neither
metadata proof created an R2 object or processed linked bytes; no CI job
success is claimed as live-source capability evidence.
...
Counts are unverified. The scoped exceptions are the dated, bounded
mediaObjects and Video Archive endpoint proofs above. Corpus-size and API
observations in the brief are otherwise hypotheses that bounded public
requests must verify; this repository does not add or generalize the 29- and
8-record endpoint counts to another source or the whole NJP Center universe.
...
| `performing-fire-corpus inventory-public --source antiegg-fluxus --max-requests 2 --ledger <path> --sanitized-manifest <path>` | `network-acquisition`, trusted VM | none; runs unauthenticated | Makes bounded public `GET` requests. | Stops on the request, timeout, rate, elapsed, or response-byte bound, on a robots restriction, or on any durable blocker. Bounds must not be raised to bypass a result. A resumed ledger replays its stored result and names the bounds it was recorded under in `stored_result_replay`. See `docs/network-acquisition-smoke.md`. |
| `performing-fire-corpus inventory-public --source antiegg-posts --max-requests 24 --max-pages 20 --ledger <path> --sanitized-manifest <path>` | `network-acquisition`, trusted VM | none; runs unauthenticated | Makes bounded public `GET` requests against `/wp-json/wp/v2/posts`; writes an ignored ledger and a sanitized manifest of hashed record ids. | Stops on the request, page, timeout, rate, elapsed, or response-byte bound, on a robots restriction, or on adapter shape drift. Reports `complete` only when unique ids retrieved equal the declared `x-wp-total`. Re-attempts on every invocation rather than replaying a stored verdict. See `docs/network-acquisition-smoke.md`. |
...
| `performing-fire-corpus record-njp-video-archive-attempt --lane <trusted-vm\|trusted-laptop> --report <path> --plan <path> --attempt-output <path> --route-output <path>` | `portable` route controller | none | Reads sanitized ignored artifacts and writes content-free, commit/plan-bound attempt and route receipts under `.local/njp-center-inventory/`; makes no network request. | VM-first. A generic transport error stays held; laptop fallback requires a separate runner-capability diagnostic digest and a closed DNS, route, HTTPS, or TLS capability-mismatch code. Policy, robots, auth, access, rate, retention, shape, MIME, bound, and elapsed blockers never fallback. |
