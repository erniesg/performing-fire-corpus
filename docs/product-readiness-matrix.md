...
| NJP Art Center site and video-archive inventory | `inventory-njp-sites [--source <id>]` | `src/performing_fire_corpus/njp_center_adapters.py`, `njp_site_inventory.py` | `tests/test_njp_center_adapters.py`, `tests/test_njp_site_inventory.py` | `network-acquisition` on the trusted VM | On 2026-07-26 the shape-bound `/mediaObjects/more` endpoint completed with 29 reachable records. On 2026-07-27 the separate Video Archive shape review completed from exact commit `4367446b`; adapter v2 is bound offline to its eight public PDF-link facts, but its bounded inventory has not yet run. See `docs/njp-center-site-inventory-report.json` and `docs/njp-center-video-archive-shape.md`. | `live-proven` for mediaObjects; `implemented-offline` plus live shape proof for Video Archive | run the independent bounded Video Archive inventory |
