...
- Confirm a dedicated proof bucket and prefix scope, all required R2 secret names as `present` or `missing`, and the required `delete_after_verification` decision without exposing values, account identifiers, endpoints, or provider details.
- Require `delete_after_verification` with an exact UTC deadline. Retention is outside this proof and must use a separately implemented and reviewed operator after issue 027.
- Require upload to one immutable content-addressed key, exact-key `HEAD` verification, deletion of only that verified key, and an exact-key absence check. No retention, listing, or broad deletion is authorized.
...
- Stop if the candidate lacks current robots allowance, explicit rights, platform permission, final URL, MIME, byte bound, proof window, dedicated scope, secret-name presence, and `delete_after_verification` decision.
