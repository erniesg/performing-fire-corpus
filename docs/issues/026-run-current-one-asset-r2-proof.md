...
Execute the existing trusted-VM one-object command only within the fresh approval from issue 025. Verify upload, immutable exact-key `HEAD`, exact-key deletion, and exact-key absence with sanitized, resumable receipts and no broader storage or source access.
...
- Revalidate approval, exact public host and URL, current robots rules, rights and platform basis, MIME, byte bound, dedicated bucket and prefix, storage scope, `delete_after_verification`, and all required secret-name presence before source acquisition.
...
- Delete only the verified exact key created by this proof and verify that exact key is absent. Retention is not supported by this operator and is outside this issue.
- Persist atomic sanitized readiness, request, upload-attempt, object, verification, cleanup, and run-manifest receipts. Resume cleanup only from matching durable exact-key receipts.
...
- At most one temporary immutable proof object created and deleted by this proof
- Exact-key verification, deletion, and absence receipt
- Sanitized aggregate proof result and exact next safe action

## Stop conditions

- Stop before network if any current approval, policy, time window, secret-name, scope, or cleanup gate is incomplete.
